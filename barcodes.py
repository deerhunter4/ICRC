...
def barcodes(proporcional_table):
    barcode_list = []
    barcode_dict = {}
    proporcional_table_insect = proporcional_table.loc[proporcional_table['label'] == 'Insect', :]
    for col_no in range(6, len(proporcional_table_insect.iloc[1, :])):
        ...
        barcode_species = proporcional_table.iloc[two_max_row_id[0], 2].split(',')[5][:-6]  # extract barcode genus name
...
def barcode_species(table, output, barcode_dict):
